...
def test_pipeline_drops_unnecessary_features(pipeline_inputs):
    ...
    assert config['drop_features'] in X_train.columns
    # When
    ...
    #Then
    assert config["drop_features"] in X_train.columns
    assert config["drop_features"] not in X_transformed.columns
...
def test_pipeline_transforms_temporal_features(pipeline_inputs):
    ...
    #Then
    assert(
        X_transformed.iloc[0]["YearRemodAdd"] == X_train.iloc[0]["YrSold"] - X_train.iloc[0]["YearRemodAdd"]
    )
